...
export default function ProfissionalId(){
    ...
    const [ profissional,setProfissional] = useState([
        {
            ...
            contato:  19999459566,
            descricao: 'Sou costureira há mais de 10 anos, faço barra, ajustes de tamanho e aplicações. Customização de peças de acordo com o desejo do cliente.',
            valores: 30
        }
    ...
    return(
        ...
        <ul className={styles.list}>
            {profissional.map((item,index) =>(<li className={styles.list} key={index}>
        ...
        <div className={styles.imagembottom}>
         ...
         <div className={styles.profissionalList}>
          <h1 className={styles.title}>{profissional[0].name}</h1>
                ...
                <p><strong>  Horario de atendimento:  </strong>{item.horario}</p>
                <p><strong> Contato: </strong>{item.contato}</p>
                <p><strong> Descrição:  </strong>{item.descricao}</p>
                <p><strong> Valores: </strong>A partir de {item.valores.toFixed(2)}</p>
        
                <button className={styles.button} onClick={() => Router.push('/chat')}>Quero contratar!</button>
